		...
		function initializeWetland($scope) {
			$('#lokalitet_wetland').typeahead({
				...
				afterSelect: function(item) {
					...
					$scope.map.fitBounds(lokalitetPolygon.getBounds(), { maxZoom: 10 } )
	  	...
	  	$scope.map.on('click', function(e) {
			...
			$scope.showPolygon = function() {
				if (lokalitet.showPolygon) {
					$scope.map.addLayer(lokalitetPolygon)
				} else {
					$scope.map.removeLayer(lokalitetPolygon)
				}
			}

			$scope.centerMarker = function() {
				$scope.map.setView(L.latLng(lokalitet.latitude, lokalitet.longitude), 11)
			}
