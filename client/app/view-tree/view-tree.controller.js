...
angular.module('dnalivApp')
	.controller('ViewTreeCtrl', ['Auth', 'User', '$rootScope', '$scope', '$filter', '$routeParams', 'Icons', 'DataModel',
		function(Auth, User, $rootScope, $scope, $filter, $routeParams, Icons, DataModel) {
			...
			$rootScope.$watch('fields.selectedCollection', function(newval, oldval) {
				if (newval !== undefined) {
					$scope.datamodels.$promise.then(function() {
						...
						if ($routeParams.treename) {
							var modelFromRoute = $scope.datamodels.filter(function(e) {
								return e.name === $routeParams.treename;
							})[0];
							if (modelFromRoute !== undefined) {
								$scope.selectedModel = modelFromRoute;
							}
